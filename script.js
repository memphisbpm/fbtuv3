...
/* ==========================================================================
   FBTU HOMEPAGE V3 CUSTOM SCRIPT ADDITIONS
   ========================================================================== */

let currentHeroSlide = 0;
let heroSliderInterval = null;

// Global slider setter for inline HTML buttons
window.setHeroSlide = function(index) {
  // Clear auto-rotation timer to avoid jumps
  ...
  // Transition slides and dot styles
...
function startHeroSliderAuto() {
  ...
  heroSliderInterval = setInterval(() => {
    ...
    setHeroSlide(next);
  }, 6500); // 6.5 seconds auto rotation
}
...
/* ── DYNAMIC LIVING MOSAIC LOOPER ────────────────────────────── */
function initLivingMosaic() {
  const cells = document.querySelectorAll('.mosaic-cell');
  if (cells.length === 0) return;
  
  // Track current index of images per cell to prevent duplicate picks
  const cellStates = Array.from(cells).map(cell => {
    const imgList = cell.getAttribute('data-images').split(',');
    return {
      element: cell,
      images: imgList,
      currentIndex: 0
  ...
  // Cycle a random cell every 2.5 seconds
  setInterval(() => {
    // 1. Pick a random cell
    const randIndex = Math.floor(Math.random() * cellStates.length);
    const cellState = cellStates[randIndex];
    
    // Check if cell is currently animating
    if (cellState.element.classList.contains('animating')) return;
    
    // 2. Choose the next image in the cycle
    cellState.currentIndex = (cellState.currentIndex + 1) % cellState.images.length;
    const nextImgSrc = cellState.images[cellState.currentIndex];
    
    // 3. Query current and next image nodes
    const currentImg = cellState.element.querySelector('.mosaic-img.current');
    const nextImg = cellState.element.querySelector('.mosaic-img.next');
    
    if (!currentImg || !nextImg) return;
    
    // 4. Prime the next image
    nextImg.src = nextImgSrc;
    
    // 5. Trigger slide animation
    cellState.element.classList.add('animating');
    
    // 6. Reset elements after transition ends (800ms)
    setTimeout(() => {
      currentImg.src = nextImgSrc;
      cellState.element.classList.remove('animating');
    }, 850);
